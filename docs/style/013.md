...
Only operators on the same chain are compared. Parenthesizing an inner pipeline makes it a separate expression. A backward pipe nested inside a lambda or a parenthesized argument of a forward chain belongs to a different expression and is left alone:
